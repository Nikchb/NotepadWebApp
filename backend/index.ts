...
import { INoteRepository, NoteRepository } from './database/repositories/noteRepository.js';
import { INoteService, NoteService } from './services/noteService.js';
import { IAuthService, AuthService } from './services/authService.js';
import { IAuthController, AuthController } from './controllers/authController.js';
import { INoteController, NoteController } from './controllers/noteController.js';
import { IUserRepository, UserRepository } from './database/repositories/userRepository.js';
...
container.addScoped<INoteRepository>('INoteRepository', async (container) => {
    return new NoteRepository(await container.get<PoolConnection>('PoolConnection'));
});

container.addScoped<IUserRepository>('IUserRepository', async (container) => {
    return new UserRepository(await container.get<PoolConnection>('PoolConnection'));
});

container.addScoped<INoteService>('INoteService', async (container) => {
    return new NoteService(await container.get<INoteRepository>('INoteRepository'));
});

container.addScoped<IAuthService>('IAuthService', async (container) => {
    return new AuthService(await container.get<IUserRepository>('IUserRepository'));
});
